...
(
    ...
    PRIMARY KEY (supervisor_id, thesis_serial_number, report_number),
    Foreign Key (report_number) REFERENCES REPORT(report_number) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY (supervisor_id) REFERENCES SUPERVISOR(id) ON DELETE CASCADE ON UPDATE CASCADE,
    Foreign Key (thesis_serial_number) REFERENCES REPORT(thesis_serial_number) ON DELETE CASCADE ON UPDATE CASCADE,
);
